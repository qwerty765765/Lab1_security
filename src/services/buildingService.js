...
const TYPE_KEYWORDS = {
  'business_center': ['бизнес', 'бц', 'офис', 'деловой', 'business', 'office'],
  'shopping_center': ['торговый', 'тц', 'трц', 'молл', 'гипермаркет', 'магазин', 'shopping', 'mall'],
  'residential': ['жилой', 'жк', 'дом', 'квартал', 'жилкомплекс', 'residential', 'house'],
  'industrial': ['завод', 'фабрика', 'производство', 'промзона', 'цех', 'industrial', 'factory'],
  'education': ['школа', 'университет', 'институт', 'колледж', 'детский сад', 'гимназия', 'school', 'university'],
  'medical': ['больница', 'поликлиника', 'медицина', 'клиника', 'госпиталь', 'hospital', 'clinic'],
  'culture': ['театр', 'музей', 'кинотеатр', 'библиотека', 'филармония', 'theater', 'museum'],
  'sport': ['стадион', 'спорткомплекс', 'фитнес', 'бассейн', 'спортзал', 'stadium', 'fitness'],
  'government': ['администрация', 'мэрия', 'правительство', 'офис', 'government', 'office'],
  'transport': ['вокзал', 'аэропорт', 'порт', 'автовокзал', 'station', 'airport'],
  'service': ['автосервис', 'ателье', 'мастерская', 'ремонт', 'service', 'workshop'],
  'warehouse': ['склад', 'логистика', 'распределительный', 'warehouse', 'storage']
};

// Функция определения типа по названию организации
...
// Функция определения типа по kind из геокодера
const detectTypeByKind = (kind) => {
  if (kind === 'house') return 'residential';
  return null;
};
...
export const detectBuildingType = async (address) => {
  try {
    ...
    if (!foundType && kind) {
      foundType = detectTypeByKind(kind);
      if (foundType) {
